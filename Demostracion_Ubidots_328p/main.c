/*
 * Demostracion_Ubidots_328p.c
 *
 * Created: 1/5/2019 14:46:05
...
#define UBIDOTS_POST "/api/v1.6/collections/values"
#define UBIDOTS_GET "/api/v1.6/variables/5cca61efc03f9740f1710e97/values?page_size=1"//
#define API_TOKEN "8dfFuD4tQXTAqsdrVpnNgzfAevbWpv"
#define TEMP_A_ID "5cca61b6c03f9741116d4a80"
#define HUM_A_ID "5cca61dbc03f9740f1710e85"
...
DEPURACION DEV=ACTIVADA;//DEPURACION ACTIVADA , cambiar para produccion
...
WiFi Cliente={
	...
	._TCP._URL=UBIDOTS_URL,
	._TCP._URL_R=UBIDOTS_POST,
	._TCP._Puerto="80",//puerto HTTP
	._TCP._Ubidots._API_TOKEN=API_TOKEN,
	._TCP._Ubidots._ID_TEMP_A=TEMP_A_ID,
	._TCP._Ubidots._ID_HUM_A=HUM_A_ID
};
/*
...
static STareas Tareas[]={
	{500 ,0 , LeerSensor},
	{1000 ,0 , LeerUbidots},
	{5000 ,0 , EnviarUbidots},
};
uint8_t Tarea_GetNumTareas(){
	return sizeof(Tareas)/sizeof(*Tareas);
}
STareas *Tarea_GetConfig(){
	return Tareas;
}
...
int main(void)
{
	...
	uint32_t Tic;
	STareas *PntrTareas;
	uint8_t IndexTarea=0;
	uint8_t NumTareas=Tarea_GetNumTareas();
	
	PntrTareas=Tarea_GetConfig();
	for(ever)
    {
		Tic=msTick;
		for (IndexTarea=0;IndexTarea<NumTareas; IndexTarea++)//Realiza un barrido por cada Tarea
		{
			if (PntrTareas[IndexTarea].Intervalo == 0)//Si hay alguna tarea cuyo intervalo sea 0; se ejecuta de forma inmediata
			{
				(*PntrTareas[IndexTarea].Funcion)();
			} 
			else if((Tic-PntrTareas[IndexTarea].AntmsTick) >= PntrTareas[IndexTarea].Intervalo)//Verifica que ha pasado el intervalo de cada tarea y la Ejecuta
			{
				(*PntrTareas[IndexTarea].Funcion)();
				PntrTareas[IndexTarea].AntmsTick=Tic;
			}
		}
    }
}
